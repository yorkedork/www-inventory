"""
An informal Apache site configuration parser.

Example usage::

    def load_vhost(self, parsed_data, raw_data):
        vhost_node = parsed_data.nodes[0]
        hosts = []
        idx = 1

        while not isinstance(vhost_node, Directive) or vhost_node.name != 'VirtualHost':
            vhost_node = parsed_data.nodes[idx]
            idx += 1
        ...
        for node in vhost_node.body.nodes:
            if not isinstance(node, Directive):
                continue

            if node.name == 'SSLEngine' and ('on' in node.arguments or 'On' in node.arguments):
                is_ssl = True
            elif node.name == 'ServerAlias':
                aliases.extend(node.arguments)
            elif node.name == 'ServerName':
                url = node.arguments[0]


    if __name__ == '__main__':
        config_path = '/some/local/path/to/a/file'
        with open(config_path, 'r') as source:
            config_fdata = source.read()
            parsed_config = ApacheConfParser(config_fdata, infile=False)
            self.load_vhost(parsed_config, config_fdata)
"""
